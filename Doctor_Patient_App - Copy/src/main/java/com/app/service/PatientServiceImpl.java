...
public class PatientServiceImpl implements PatientServiceIntf {
	...
	/*
	 * @Override public Patient savePatient(PatientDTO patient) { String email =
	 * patient.getEmail(); Doctor doctor = doctorRepo.findByEmail(email).get();
	 * Admin admin = adminRepo.findByEmail(email).get();
	 * 
	 * if(doctor == null || admin == null) { throw new
	 * UserHandlingException("Email already exists!!!"); }
	 * 
